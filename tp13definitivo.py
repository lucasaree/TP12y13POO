...
class VentanaEconomica:
    ...
    def abrir_ventana_costo_oportunidad(self):
        ...
        Label(ventana_costo, text="Ingrese el valor del beneficio del bien sacrificado:").pack()
        self.beneficio_sacrificado = Entry(ventana_costo)
        self.beneficio_sacrificado.pack(pady=5)
        
        Label(ventana_costo, text="Ingrese el valor del bien sacrificado:").pack()
        self.valor_sacrificado = Entry(ventana_costo)
        self.valor_sacrificado.pack(pady=5)
        
        Button(ventana_costo, text="Calcular Costo de Oportunidad", command=self.calcular_costo_oportunidad, font=("Helvetica", 14), width=20, bg="#99ccff").pack(pady=10)
    ...
    def calcular_costo_oportunidad(self):
        try:
            beneficio = float(self.beneficio_sacrificado.get())
            valor = float(self.valor_sacrificado.get())
            costo_oportunidad = beneficio * valor
            messagebox.showinfo("Resultado", f"Costo de Oportunidad: {costo_oportunidad}")
        except ValueError:
            messagebox.showerror("Error", "Ingrese valores numéricos válidos para el beneficio y el valor.")
